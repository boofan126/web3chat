...
const express=require("express"),crypto=require("crypto"),fs=require("fs"),path=require("path"),lockfile=require("proper-lockfile"),router=express.Router();router.use(express.json({limit:"256kb"}));
...
async function withLock(name,fn){const lk=path.join(DATA,name+".lock");const release=await lockfile.lock(lk,LOCK_OPTS);try{return await fn()}finally{try{await release()}catch(e){}}}
...
router.post("/pubkey",async(r,t)=>{const{addr:s,sign:e,dh:o,nick:n,ts:i,sig:a}=r.body||{};if(!(s&&e&&o&&a))return t.status(400).json({ok:false,err:"missing_fields"});let u;try{u=deriveAddress(e)}catch(r){return t.status(400).json({ok:false,err:"bad_sign_pub"})}if(u!==s)return t.status(403).json({ok:false,err:"addr_mismatch"});if(!await verifySig(e,CHALLENGE(s,i),a))return t.status(403).json({ok:false,err:"bad_sig"});await withLock("pubkeys",async()=>{const d=load(PUBKEYS);d[s]={addr:s,sign:e,dh:o,nick:(n||"").slice(0,40),ts:i||Date.now()};save(PUBKEYS,d)});t.json({ok:true})});
...
router.post("/backup",async(r,t)=>{const{id:s,ct:e,sign:sg,ts:i,sig:a}=r.body||{};if(!s||!e||"string"!=typeof e||e.length>2e5)return t.status(400).json({ok:false,err:"bad"});if(!authSigned(sg,s+"|"+i+"|"+e,a,s,i))return t.status(403).json({ok:false,err:"bad_sig"});await withLock("backups",async()=>{const o=load(BACKUPS);o[String(s)]={id:String(s),ct:e,ts:Date.now()};save(BACKUPS,o)});t.json({ok:true})});
...
router.post("/wake",async(r,t)=>{
  ...
  await withLock("wakes",async()=>{const w=load(WAKES);w[s]=o;save(WAKES,w)});
...
router.get("/wake",async(r,t)=>{
  ...
  const o=e[s];
  await withLock("wakes",async()=>{const w=load(WAKES);delete w[s];save(WAKES,w)});
...
router.post("/wake/sub",async(r,t)=>{
  ...
  if(!authSigned(sg,s+"|"+i,a,s,i))return t.status(403).json({ok:false,err:"bad_sig"});
  await withLock("wakesubs",async()=>{const subs=load(WAKESUBS);if(sub){subs[s]=sub}else{delete subs[s]} save(WAKESUBS,subs);});
  t.json({ok:true});
});

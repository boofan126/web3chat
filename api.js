...
router.post("/backup",(r,t)=>{const{id:s,ct:e}=r.body||{};if(!s||!e||"string"!=typeof e||e.length>2e5)return t.status(400).json({ok:false,err:"bad"});const o=load(BACKUPS);o[String(s)]={id:String(s),ct:e,ts:Date.now()},save(BACKUPS,o),t.json({ok:true})});
router.get("/backup",(r,t)=>{const s=r.query.id;if(!s)return t.status(400).json({ok:false,err:"missing_id"});const e=load(BACKUPS)[String(s)];if(!e)return t.status(404).json({ok:false,err:"not_found"});t.json({ok:true,id:e.id,ct:e.ct,ts:e.ts})});
...
router.post("/wake",(r,t)=>{
  const{addr:s,from:e,mid:m}=r.body||{};
...
router.get("/wake",(r,t)=>{
  const s=r.query.addr; if(!s)return t.status(400).json({ok:false,err:"missing_addr"});
...
router.post("/wake/sub",(r,t)=>{
  const{addr:s,subscription:sub}=r.body||{};
  if(!s)return t.status(400).json({ok:false,err:"missing_addr"});
